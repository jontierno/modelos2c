...
for x in range(COLUMNA_FECHA_1, COLUMNA_FECHA_1+FECHAS):
	with open('GranDT2015_formatted_python.csv', newline='') as csvfile:
		...
		suma343 = sum(int(row[x]) for row in trescuatrotres)

		datos442 = datos.copy()
		cuatrocuatrodos = generarEquipo(datos442, 4, 4, 2)
		suma442 = sum(int(row[x]) for row in cuatrocuatrodos)

		datos433 = datos.copy()
		cuatrotrestres = generarEquipo(datos433, 4, 3, 3)
		suma433 = sum(int(row[x]) for row in cuatrotrestres)
		...
		puntaje = sum(int(row[x]) for row in formacionSel)
